...
let options4 = Array.from(selectElement4.options);  // Chuyển HTMLCollection thành mảng
let selected = false;

// Kiểm tra và chọn file có tên hiển thị là "Ban_Xem_Truoc.pdf"
for (let option of options4) {
    if (option.text.trim() === "Ban_Xem_Truoc.pdf") {  // Kiểm tra tên hiển thị
        selectElement4.value = option.value;  // Cập nhật giá trị của select
        selectElement4.dispatchEvent(new Event('change'));  // Gửi sự kiện 'change'
        console.log("Đã chọn file Ban_Xem_Truoc.pdf: " + option.value);
        selected = true;
        break;  // Dừng vòng lặp khi đã chọn file
    }
}

// Nếu không tìm thấy file "Ban_Xem_Truoc.pdf", chọn file khác
if (!selected) {
    let pdfFound = false;
    for (let option of options4) {
        if (option.value && option.value.endsWith('.pdf')) {  // Kiểm tra tất cả các file có đuôi ".pdf"
            selectElement4.value = option.value;  // Cập nhật giá trị của select
            selectElement4.dispatchEvent(new Event('change'));  // Gửi sự kiện 'change'
            console.log(`Đã chọn file PDF: ${option.value}`);
            pdfFound = true;
            break;  // Chọn file đầu tiên có đuôi .pdf
        }
    }

    // Nếu không tìm thấy file .pdf nào, chọn giá trị mặc định
    if (!pdfFound) {
        selectElement4.selectedIndex = 0;  // Chọn giá trị đầu tiên nếu không tìm thấy file PDF
        selectElement4.dispatchEvent(new Event('change'));
        console.log("Không tìm thấy file PDF nào. Đã chọn giá trị mặc định.");
    }
}
        ...
        if (!isValidFileSelected) {
            ...
            watermark.textContent = 'FILE BẢN CHÍNH CỦA BẠN KO PHẢI .DOC , .DOCX, .ZIP, NẾU BẢN CHÍNH CỦA BẠN LÀ FILE PDF,BẠN HÃY CHỌN ĐÚNG FILE PDF XEM TRƯỚC Ở PHẦN "UPLOAD PDF" VÀ BẠN HÃY CHỌN ĐÚNG FILE PDF BẢN CHÍNH Ở MỤC "CHỌN TÀI LIỆU", ';
    ...
    (function() {
        ...
        watermark.textContent = 'KHỞI TẠO BUTTON THÀNH CÔNG (update 7:38 [AM] 15/12/2024) ducanhwork26';  // Nội dung watermark
